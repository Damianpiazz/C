...
int main() {
    ...
    do {
        printf("\n--- Concesionaria: Gestión de Autos ---\n");
...
// Función para buscar un auto por su ID
void buscarAutoPorID(FILE *autoFile, int idAuto) {
    Auto autoActual;
    int found = 0;
    ...
    if (!found) {
        printf("No se encontró el auto con ID: %d\n", idAuto);
    }
}

// Función para actualizar el stock de un auto por su ID
void actualizarStockPorID(FILE *autoFile, int idAuto, int stockNuevo) {
    Auto autoActual;
    ...
    if (fread(&autoActual, sizeof(Auto), 1, autoFile)) {
        ...
        // Mover el puntero nuevamente a la posición del registro para actualizarlo
