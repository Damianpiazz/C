...
// Definimos una estructura para almacenar la información de una persona
typedef struct {
    int id;             // ID único
...
// Función para calcular el hash SHA-256 de un valor (como un ID)
unsigned int sha256Hash(int id) {
    ...
    // Tomamos los primeros 4 bytes del hash y los usamos como índice (32 bits)
    unsigned int index = 0;
    for (int i = 0; i < 4; i++) {
        index = (index << 8) | hash[i];  // Convertimos los primeros 4 bytes en un número
    }

    // Usamos el módulo para limitar el índice al tamaño de la tabla
...
void insert(Persona p) {
    ...
    // Si no hay colisión, simplemente insertamos el nodo en esa posición
    if (hashTable[index] == NULL) {
        hashTable[index] = newNode;
    } else {
        // Si hay una colisión, agregamos el nuevo nodo al principio de la lista enlazada
...
Persona* search(int id) {
    ...
    // Recorremos la lista enlazada en ese índice para encontrar la persona con el ID
